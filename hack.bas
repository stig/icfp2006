...
D        DIM k AS INTEGER
DI       DIM pass AS STRING


DIII    j = I
DV      k = I
DX      pass = words(i) + CHR(XLVII + j) + CHR(XLVII + k)
DC      IF CHECKPASS(username, pass) THEN GOTO M

DCV     k = k + I
DCX     IF k < XI THEN GOTO DX
DCL     j = j + I
DCC     IF j < XI THEN GOTO DV
DCCV    i = i + I
DCCX    IF i > pwdcount THEN GOTO MIII

DCCXI    GOTO DIII
M       PRINT "found match!! for user " + username + CHR(X)
MI      PRINT "password: " + pass + CHR(X)
MII     END
MIII    PRINT "no complex matches for user " + username + CHR(X)
